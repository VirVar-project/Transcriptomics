...
files<-list.dirs("kallisko_results") %>% .[-1] %>% file.path(.,"abundance.h5") #Not normalized file

samples<-str_remove(files, "/abundance.h5") %>% str_remove("kallisko_results/") #Clean up file, string remove
...
metadata<-readxl::read_xlsx("Metadata.xlsx") # Read in metadata
metadata<-readxl::read_xlsx("Metadata_2.xlsx") # Read in metadata
